...
class Backend():
    ...
    def CreateTables(self):
        self.db.connect()
        self.db.drop_tables([FilterAgent])
    ...
    def LoadAnnotations(self, images):

        self.plugins = ["test-agent"]
        images = Image.select()
        self.pe.reload_plugins()
        for im in images:
            self.pe.start_annotations(im.Path, self.plugins)
...
class FolderData():
    ...
    def open_db(self, settings, windowManager):
        self.openFolderPath = "db"
        self.settings = settings
        self.openFolderData = self.get_db_folder_data(self.openFolderPath)
        return True
    ...
    def get_db_folder_data(self, folderPath):
        return self.new_db_data(folderPath)
    ...
    def new_folder_data(self, folderPath):
        ...
        folderData = {
            ...
            'imageFullPath': {
                os.path.join(folderPath, img): self.new_image_data(img)
                for img in list_images(folderPath)
            },
        }
        self.backend.FillImages(folderData['imageFullPath'])
        # Make new dirs
    ...
    def new_db_data(self, folderPath):
        folderData = {
            'path': "db",
            'uid':  "db",
            'thumbnailFolder': os.path.join('.metadata', "db", self.thumbsSubDir),
            'backupsFolder': os.path.join('.metadata', "db", self.backupsSubDir),
            'shortName': "db",
            'imageData': {
                img.Path
...
class ThreadedThumbApp(threading.Thread):
    ...
    def run(self):
        for img in self.images:
            try:
                PILIM.open(self.wm.folderData.thumb_path(img))
...
class ThreadedDbThumbApp(threading.Thread):
    """ Retrieve thumbnails in a thread parallel to main window thread """

    def __init__(self, windowManager, dest, images):
        super().__init__()
        self._stop_event = threading.Event()
        self.wm = windowManager
        self.dest = dest
...
class WindowManager():
    ...
    collectionSelectedKey = 'collection_selected'
    annotationButtonKey = 'annotation_button'
    dbButtonKey = 'db_button'
    thumbSize = 'S'
    ...
    def window_event_loop(self):
        while True:
            ...
            if event == self.collectionSelectedKey:
                if self.collectionSelectedKey:
                    selected_indexes = self.window[self.collectionSelectedKey].get_indexes(
                    )
                    all_items = self.window[self.collectionSelectedKey].get_list_values(
                    )
                    selected_items = [all_items[index]
                                      for index in selected_indexes]
                    self.openSelectedCollection(selected_items)
            if event == self.annotationButtonKey:
                selected = self.get_selected_images()
                if selected.count == 0:
                    selected = Image.get()

            if event == self.dbButtonKey:
                self.folder = self.folderData.open_db(
                    settings=self.folderSettings,
                    windowManager=self,
                )
                self.sortByRating = False
                self.kickoff_thumb_db_threads()
    ...
    def kickoff_thumb_db_threads(self):
        dest = self.folderData.openFolderData['thumbnailFolder']
        ThreadedDbThumbApp(self, dest, list_images_db()).start()
    ...
    def update_star_display(self, image):
        ...
        for i in range(4):
            ...
            if rating >= i:
                self.window[ik].update(
                    'C:\\UniRepos\\Algo-Fil\\Algo-Fil.Services\\Plugins\\imgs\\full_star.png')
            else:
                self.window[ik].update(
                    'C:\\UniRepos\\Algo-Fil\\Algo-Fil.Services\\Plugins\\imgs\\empty_star.png')
    ...
    def openSelectedCollection(self, collection):
        sg.popup(
            f'Opened { collection }',
            title='Success!'
        )
    ...
    def menu_layout(self):
        ...
        annotationsFrame = sg.Frame(
            'Run annotations',
            [[
                sg.Button('Annotated selected', key=self.annotationButtonKey,
                          enable_events=True)
            ]]
        )

        loadFromDbFrame = sg.Frame(
            'Load DB',
            [[
                sg.Button('Load DB', key=self.dbButtonKey,
                          enable_events=True)
            ]]
        )
        return [
            sortFrame, moveFrame, openGalleryFrame,
            sg.InputText(key=self.selectFolderKey,
                         enable_events=True, visible=False), annotationsFrame, loadFromDbFrame
        ]
    ...
    def gallery_element(self, elemData):
        ...
        ratingStars = [
            sg.Image(
                'C:\\UniRepos\\Algo-Fil\\Algo-Fil.Services\\Plugins\\imgs\\full_star.png' if i <= rating else 'C:\\UniRepos\\Algo-Fil\\Algo-Fil.Services\\Plugins\\imgs\\empty_star.png',
                enable_events=True,
                pad=(2, 0),
    ...
    def layout(self):
        col1 = [[sg.Listbox(values=[
                'Welcome Drink', 'Extra Cushions', 'Organic Diet', 'Blanket', 'Neck Rest'], select_mode='single', size=(100, sg.Window.get_screen_size()[1]-100), key=self.collectionSelectedKey, enable_events=True)]]
        col2 = [self.menu_layout(), [sg.Column(
            self.gallery_layout(),
            size=(
        ...
        return [[sg.Column(col1, element_justification='c', size=(100, sg.Window.get_screen_size()[1]-100)), sg.Column(col2, element_justification='c')]]
